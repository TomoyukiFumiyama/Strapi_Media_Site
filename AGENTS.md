...
## プロンプトログ運用ルール
- ユーザーからこのリポジトリに対する作業指示を受けた場合、`.agents/PROMPTS.md` にログとして追記する。
- 追記内容は、日時（UTC）、依頼概要、ユーザープロンプト原文を含める。
- `.agents/PROMPTS.md` は指示ログのみを管理し、完了履歴は `.agents/CHANGELOG.md` に記録する。
- 秘密情報・APIキー・パスワードがプロンプトに含まれる場合は、値を伏せ字にして記録する。
